 ...
 * // Puedes utilizar consultas SQL, JPA o cualquier otro mecanismo de
 * persistencia
 * 
 * List<Usuario> usuarios = new ArrayList<>();
 * 
 ...
 * case "cliente":
 * query =
 * "SELECT u.*, c.* FROM usuarios u JOIN clientes c ON u.id = c.usuario_id";
 * break;
 * case "administrativo":
 * query =
 * "SELECT u.*, a.* FROM usuarios u JOIN administrativos a ON u.id = a.usuario_id"
 * ;
 * break;
 * case "profesional":
 * query =
 * "SELECT u.*, p.* FROM usuarios u JOIN profesionales p ON u.id = p.usuario_id"
 * ;
 * break;
 * default:
 * throw new IllegalArgumentException("Tipo de usuario no válido");
 ...
 * return DriverManager.getConnection("tu_url_de_conexion", "usuario",
 * "contraseña");
 * }
 * }
 * 
